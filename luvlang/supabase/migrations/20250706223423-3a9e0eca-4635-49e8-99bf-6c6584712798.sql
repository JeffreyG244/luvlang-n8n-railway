-- Temporarily remove foreign key constraint for compatibility_answers to allow seed data
ALTER TABLE compatibility_answers DROP CONSTRAINT IF EXISTS compatibility_answers_user_id_fkey;

-- Create compatibility answers for all existing seed profiles
...
SELECT 
  ...
  jsonb_build_object(
    ...
    '12', CASE 
      ...
      ELSE 'Everyone'
    END,
    -- Add personality traits for better matching
...
WHERE dp.user_id NOT IN (SELECT user_id FROM compatibility_answers WHERE user_id IS NOT NULL);