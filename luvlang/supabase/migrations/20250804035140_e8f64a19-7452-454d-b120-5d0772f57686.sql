-- ============================================
-- COMPLETE LUVLANG DATABASE RESET & RECREATION
-- ============================================

-- STEP 1: DROP ALL EXISTING TABLES (BE CAREFUL!)
-- ============================================

-- Drop all foreign key dependent tables first
...
-- STEP 2: CREATE EXECUTIVE DATING PLATFORM SCHEMA
-- ============================================

-- Enhanced Users Table (Core Profile)
...
-- Heaven-Level Enhanced Matches
CREATE TABLE public.ai_enhanced_matches (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES users(id) ON DELETE CASCADE,
  ...
  relationship_potential DECIMAL(5,2) DEFAULT 0, -- Marriage potential
  success_probability DECIMAL(5,2) DEFAULT 0, -- 6+ month relationship chance
...
CREATE TABLE public.ai_icebreakers (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  match_id UUID REFERENCES ai_enhanced_matches(id) ON DELETE CASCADE,
...
CREATE TABLE public.conversations (
  ...
  match_id UUID REFERENCES ai_enhanced_matches(id),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
CREATE TABLE public.proximity_alerts (
  ...
  time_window INTEGER DEFAULT 30, -- minutes
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  expires_at TIMESTAMP WITH TIME ZONE,
...
CREATE TABLE public.success_stories (
  ...
  story TEXT NOT NULL,
  relationship_milestone VARCHAR(100), -- engaged, married, moved_in_together
...
-- Matching indexes
CREATE INDEX idx_matches_compatibility ON ai_enhanced_matches(compatibility_score DESC, created_at DESC);
CREATE INDEX idx_matches_users ON ai_enhanced_matches(user_id, matched_user_id);
CREATE INDEX idx_matches_status ON ai_enhanced_matches(status, created_at DESC);
...
-- Proximity
CREATE INDEX idx_proximity_user_time ON proximity_alerts(user_id, created_at DESC, expires_at);
...
ALTER TABLE ai_enhanced_matches ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Users can view matches they're part of" ON ai_enhanced_matches
  FOR SELECT USING (auth.uid() = user_id OR auth.uid() = matched_user_id);

CREATE POLICY "System can create matches" ON ai_enhanced_matches
  FOR INSERT WITH CHECK (true);

CREATE POLICY "Users can update their match actions" ON ai_enhanced_matches
