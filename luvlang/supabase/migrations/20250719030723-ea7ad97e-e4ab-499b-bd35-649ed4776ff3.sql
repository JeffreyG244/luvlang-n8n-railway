...
-- Create a profiles table for additional user data (since dating_profiles exists but user profile is missing)
...
SELECT 
  ...
  now() as updated_at
FROM auth.users 
WHERE id NOT IN (SELECT user_id FROM public.dating_profiles WHERE user_id IS NOT NULL)
