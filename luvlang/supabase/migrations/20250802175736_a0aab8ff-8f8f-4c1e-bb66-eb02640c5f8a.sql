...
-- Add missing policies for tables without them
CREATE POLICY "mutual_matches_read_access" ON "public"."mutual_matches" 
AS PERMISSIVE FOR SELECT 
TO authenticated 
USING (
  profile_id_1 IN (
    SELECT user_id FROM dating_profiles WHERE user_id = auth.uid()
  ) OR 
  profile_id_2 IN (
    SELECT user_id FROM dating_profiles WHERE user_id = auth.uid()
  )
