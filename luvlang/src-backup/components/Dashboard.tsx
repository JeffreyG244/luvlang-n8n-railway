...
const MatchesView = () => {
  ...
  const loadMatches = async () => {
    try {
      ...
      // Get user's profile for matching preferences
      ...
      const { data: dbProfiles, error: dbError } = await supabase
        ...
        .limit(20); // Get more to filter by preferences
      ...
      if (dbProfiles && dbProfiles.length > 0) {
        ...
        allProfiles = dbProfiles.map(profile => ({
          ...
          profile_image_url: profile.gender === 'female' ? '👩‍💼' : '👨‍💼'
        }));
      } else {
      ...
      // COMPREHENSIVE SEXUAL ORIENTATION AND GENDER MATCHING
      const userGender = userProfile?.gender;
      const sexualOrientation = userProfile?.profile_data?.sexualOrientation || [];
      const interestedIn = userProfile?.profile_data?.interestedIn || [];
      
      // Smart matching function that respects all orientations
      const isCompatibleMatch = (profile) => {
        const profileGender = profile.gender;
        const profileOrientation = profile.profile_data?.sexualOrientation || [];
        const profileInterestedIn = profile.profile_data?.interestedIn || [];
        
        // If user hasn't set preferences, use conservative matching
        if (!userGender || (!sexualOrientation.length && !interestedIn.length)) {
          return true; // Show all for now, will filter later
        }
        
        // Check user's orientation preferences
        for (const orientation of sexualOrientation) {
          switch (orientation) {
            case 'Straight/Heterosexual':
              if (userGender === 'male' && ['female', 'transgender-female'].includes(profileGender)) {
                // Also check if profile is interested in men
                if (profileInterestedIn.includes('Men') || profileOrientation.includes('Straight/Heterosexual')) {
                  return true;
                }
              }
              if (userGender === 'female' && ['male', 'transgender-male'].includes(profileGender)) {
                // Also check if profile is interested in women
                if (profileInterestedIn.includes('Women') || profileOrientation.includes('Straight/Heterosexual')) {
                  return true;
                }
              }
              break;
            ...
            case 'Gay':
              if (userGender === 'male' && ['male', 'transgender-male'].includes(profileGender)) {
                // Check if profile is also interested in men
                if (profileInterestedIn.includes('Men') || profileOrientation.includes('Gay')) {
                  return true;
                }
              }
              break;
            ...
            case 'Lesbian':
              if (userGender === 'female' && ['female', 'transgender-female'].includes(profileGender)) {
                // Check if profile is also interested in women
                if (profileInterestedIn.includes('Women') || profileOrientation.includes('Lesbian')) {
                  return true;
                }
              }
              break;
            ...
            case 'Pansexual':
            case 'Queer':
              // These orientations are open to multiple genders
              if (interestedIn.includes('Anyone') || interestedIn.length > 1) {
                return true;
              }
              // Check specific gender interests
              if (interestedIn.includes('Men') && ['male', 'transgender-male'].includes(profileGender)) {
                return true;
              }
              if (interestedIn.includes('Women') && ['female', 'transgender-female'].includes(profileGender)) {
                return true;
              }
              if (interestedIn.includes('Non-binary people') && ['non-binary', 'genderfluid'].includes(profileGender)) {
                return true;
              }
              break;
            ...
            case 'Demisexual':
            case 'Asexual':
              // These can be combined with other orientations, check "interestedIn"
              return checkInterestedInCompatibility();
        ...
        // Check "interested in" preferences if no orientation match
        function checkInterestedInCompatibility() {
          if (interestedIn.includes('Anyone')) return true;
          ...
          if (interestedIn.includes('Trans men') && profileGender === 'transgender-male') {
            return true;
          }
          if (interestedIn.includes('Trans women') && profileGender === 'transgender-female') {
            return true;
          }
      ...
      if (userGender && (sexualOrientation.length > 0 || interestedIn.length > 0)) {
        const compatibleProfiles = allProfiles.filter(isCompatibleMatch);
        
        // If we have compatible matches, use those
        if (compatibleProfiles.length >= 3) {
          allProfiles = compatibleProfiles;
        } else {
          // Fall back to broader matching to ensure some profiles
          console.log('Limited compatible matches, showing broader selection');
        }
      }
      
      // Default fallback for users without preferences set
      if (!userGender || (!sexualOrientation.length && !interestedIn.length)) {
        // Show diverse mix but ensure balanced representation
        const femaleProfiles = allProfiles.filter(p => p.gender === 'female');
        const maleProfiles = allProfiles.filter(p => p.gender === 'male');
        allProfiles = [...femaleProfiles.slice(0, 4), ...maleProfiles.slice(0, 4)];
      }
